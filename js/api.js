...
(function () {
  ...
  api.fetchHistory = () =>
    request(REST + "/words?assigned_date=not.is.null&select=id,finnish,swedish,assigned_date&order=assigned_date.desc");

  /* ── admin ── */
  api.fetchQueue = () =>
    request(REST + "/words?assigned_date=is.null&select=id,finnish,swedish,created_at&order=created_at");

  api.fetchScheduled = () =>
    request(REST + "/words?assigned_date=gt." + todayISO() + "&select=id,finnish,swedish,assigned_date&order=assigned_date");
